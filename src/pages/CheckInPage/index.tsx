...
const CheckInPage: React.FC = () => {
  ...
  const coderef = searchParams.get("coderef");
  const codeauth = searchParams.get("codeauth");
  const validate = () => {
    ...
    if (!formData.idCard) newErrors.idCard = "CCCD/CMND là bắt buộc";
     ...
     toast.error("CCCD/CMND phải có 9 hoặc 12 chữ số");
  }
  ...
  useLayoutEffect(() => {
    ...
    getProvinces("VN")
      getProvincesOld("VN")
    document.title = "Đăng ký lượt khám";
  }, []);
      ...
      if (data.data.client.district_id) {
        getWards(data.data.client.district_id);
          ...
          if (data.data.allow_update_profile === false) {
              navigate(`/verify-profile?coderef=${coderef}&codeauth=${codeauth}`)
          }
          setLoading(false)
        }
        else {
...
>
    ...
    <circle cx="50" cy="80" r="5" fill="white" />
       ...
       {" "}    Để tiết kiệm thời gian của quý khách khi đến khám, vui lòng điền đầy
          đủ thông tin sau đây:
        </p></div>
      </div>
...
/>
        ...
        <div style={{ marginBottom: "8px" }}>
          ...
          >
            ...
            <span style={{ fontSize: "12px" }}>
              (vui lòng nhập đúng địa chỉ để nhận kết quả sau khám)
            </span>
          </label>
          <Input.TextArea
            name="address"
        ...
        >
          ...
          >
        <img src={logoSave} style={{ width: "18px" }}></img> Đăng ký
          </Button> 
